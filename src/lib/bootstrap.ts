/**
 * 완전히 새로운 데이터 폴더(DATA_DIR)로 처음 실행됐을 때(org.db가 아직 없거나
 * 비어있을 때) 최소한의 상태를 만들어준다: 스키마 적용 + 로그인할 수 있는
 * 계정이 하나도 없으면 기본 관리자 계정을 하나 만든다. 이게 없으면 org.db가
 * 비어있는 상태로 첫 로그인 시도 자체가 "테이블이 없습니다" 에러로 실패한다.
 *
