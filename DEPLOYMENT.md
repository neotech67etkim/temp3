...
## 빠른 설치/실행: `server-start.bat` (Windows)

저장소 루트의 `server-start.bat`가 아래 "최초 설치 절차"와 "업데이트(재배포)"를
한 번에 처리합니다: `git pull` → `npm ci` → (`.env`가 없으면 최초 1회 물어봐서
생성) → 빌드 → pm2로 서버 실행/재시작.

```powershell
server-start.bat
```

최초 실행 시에는 데이터 폴더 경로와 접속 주소를 물어보고, 그 뒤로는 그냥
다시 실행하면(업데이트할 때마다) 알아서 최신 코드로 재시작해줍니다. pm2가
없으면 자동으로 설치합니다. (Windows 작업 스케줄러에 재부팅 시 자동 시작
등록은 이 스크립트가 대신 해주지 않으니, 아래 "상시 실행 유지" 절을 별도로
한 번 확인하세요.)

수동으로 단계별 진행하려면 아래를 참고하세요.
...
`DATA_DIR`은 서버가 꺼졌다 켜져도 그대로 있어야 하는 폴더입니다(내장/외장
디스크처럼 이 PC에 직접 연결된 드라이브면 됩니다).

> **주의**: `DATA_DIR`을 `V:\...`처럼 네트워크 공유 드라이브(NAS, UNC 경로)로
> 지정하면 안 됩니다. SQLite는 네트워크 드라이브 위에서 파일 잠금이
> 불안정해서 `Socket timeout ... P1008` 같은 오류가 납니다(예전 Electron
> 버전이 "로컬로 복사해서 편집 후 NAS로 반영"하는 절차를 따로 뒀던 이유가
> 바로 이것입니다). 반드시 서버 PC의 로컬 디스크 경로를 쓰세요. 기존에
> NAS/공유 폴더에 있던 `org.db`/과별 `.db` 파일이 있다면, 로컬 디스크로
> 복사해 넣고 그 로컬 경로를 `DATA_DIR`로 지정하세요(예: `robocopy
> "V:\기존 경로" "C:\workorder-data" /E`). 네트워크 드라이브는 정기 백업
> 대상으로만 쓰세요.
...
(저장소 루트의 `ecosystem.config.js`가 실행할 스크립트와 인자를 미리
정의해둡니다. `pm2 start npm -- start`나 `pm2 start ... -- start -p 4200`처럼
인자를 커맨드라인으로 직접 넘기면, 일부 pm2 버전이 Windows에서 그 인자
자체를 스크립트 이름으로 잘못 해석하는 버그가 있어("Script not found") 이
방식으로 우회합니다.)
